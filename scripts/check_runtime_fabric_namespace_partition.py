...
TOP_FIELDS = {
    "profile_id", "profile_version", "status", "authority_effect",
    "decision_scope", "prohibited_promotions", "legacy_collisions",
    "required_semantic_classes", "candidate_profiles", "mandatory_fields",
    "invariants", "required_witnesses", "migration_requirements",
    "rollback_requirements", "review_gates", "fysa_120",
}
EXPECTED_CLASSES = {
    "runtime_event", "runtime_execution_report", "fabric_projection_receipt",
    "fabric_collaboration_event", "fabric_aggregate_report",
...
REQUIRED_FIELDS = {
    "profile_id", "profile_version", "semantic_namespace", "semantic_class",
    "producer_id", "producer_role", "producer_generation", "record_id",
    "record_identity_inputs", "subject_id", "subject_generation",
    "source_record_ids", "source_set_digest", "event_time",
    "observation_time", "trusted_time_status", "local_sequence",
    "causal_parents", "ordering_scope", "duplicate_disposition",
    "replay_disposition", "idempotency_key", "conflict_disposition",
    "transformation_or_aggregation_rule_id",
    "projection_or_aggregation_receipt_id", "privacy_classification",
    "retention_classification", "correction_refs", "revocation_refs",
    "supersession_refs", "withdrawal_refs", "consumer_profile",
    "unsupported_version_behavior", "authority_effect", "rollback_target",
    "restored_state_verification",
}
EXPECTED_ROUTES = {
    "README.md", "docs/index.md",
    "docs/runtime-fabric-namespace-partition.md", "taskchain.md",
    "release.md", "punchlist.md", "changelog.md", "mkdocs.yml",
}
PROHIBITED = {
    "canonical_namespace_selection", "schema_acceptance",
    "consumer_registration", "runtime_admission", "fabric_activation",
    "repository_1_authority", "credential_or_capability_creation",
    "merge_to_default_branch", "release", "pages_publication", "deployment",
    "infrastructure_apply",
}
REVIEW_GATES = {
    "D1_ACCEPTED", "D2_ACCEPTED", "D3_ACCEPTED",
    "EXACT_REPOSITORY_LOCAL_INVENTORY_COMPLETE",
    "PARTITION_PROFILE_SELECTED", "SEMANTIC_CLASSES_IMMUTABLE",
    "SECURITY_REVIEW_COMPLETE", "PRIVACY_REVIEW_COMPLETE",
    "ACCESSIBILITY_REVIEW_COMPLETE", "LICENSE_REVIEW_COMPLETE",
    "ARCHITECTURE_REVIEW_COMPLETE",
    "TWO_INDEPENDENT_CONSUMERS_OR_VALIDATORS_PASS",
    "MIGRATION_AND_ROLLBACK_EVIDENCE_COMPLETE", "EXPLICIT_HUMAN_APPROVAL",
    "RESULTING_STATE_VERIFIED",
}
REQUIRED_CATEGORIES = {
    "CAT-011", "CAT-012", "CAT-013", "CAT-017", "CAT-019",
    "CAT-031", "CAT-032", "CAT-040", "CAT-052", "CAT-070",
}
...
def validate_packet(
    ...
    if set(unique_strings(packet["prohibited_promotions"], "prohibited_promotions")) != PROHIBITED:
    ...
    for item in collisions:
        ...
        collision_map[item["label"]] = set(
            unique_strings(item["candidate_meanings"], "candidate_meanings")
        )
    if collision_map != {
    ...
    for item in classes:
        if not isinstance(item, dict) or set(item) != {
            "id", "candidate_producer", "authority_effect"
        }:
            raise ValueError("semantic class entries must use the closed schema")
        class_ids.add(item["id"])
        expected_effect = (
            "SEPARATE_APPROVED_AUTHORITY_REQUIRED"
            if item["id"] == "portfolio_disposition"
            else "NONE"
        )
        if item["authority_effect"] != expected_effect:
            raise ValueError(f"invalid authority effect for {item['id']}")
    ...
    for item in profiles:
        if not isinstance(item, dict) or set(item) not in allowed_profile_fields:
    ...
    mandatory_fields = set(unique_strings(packet["mandatory_fields"], "mandatory_fields"))
    ...
    for item in witnesses:
        ...
        if len(unique_strings(item["requirements"], "witness requirements")) < 4:
    ...
    if len(unique_strings(packet["migration_requirements"], "migration_requirements")) < 8:
        raise ValueError("migration requirements are incomplete")
    if len(unique_strings(packet["rollback_requirements"], "rollback_requirements")) < 8:
        raise ValueError("rollback requirements are incomplete")
    if set(unique_strings(packet["review_gates"], "review_gates")) != REVIEW_GATES:
    ...
    for route in sorted(EXPECTED_ROUTES):
        ...
        if "runtime/fabric" not in folded or "blocked_role_collision" not in folded:
