...
- [x] Compare isolated Repository `1`, split issuance/recovery, and federated human-reviewed quorum models without selecting one.
- [x] Record observed Repository `1` PR #2 at `432a6fafa56d4a57be7fc3918eba5ed80a6bcdcc` and PR #1 at `0813308061e27e8289ea8f15af7d5ccdc84b4abf` as candidate evidence only.
- [x] Record the `working → proposal → quarantine` versus direct `working → quarantine` route bifurcation.
- [x] Define required authority, capability, receipt, correction, revocation, checkpoint, recovery, consumer-acknowledgment, and restored-state records.
- [x] Define pairwise and triple-overlap witness requirements.
- [x] Document constitutional-loop, self-approval, root-co-location, revocation-orphan, checkpoint-fork, compromised-root recovery, stale-quorum, replay, rollback-resurrection, and unverified-restoration obstructions.
- [ ] Accept D1–D3 dependencies.
- [ ] Select a D4 model and accepted constitutional source.
- [ ] Assign issuer, approver, revoker, auditor, incident, key-custody, checkpoint, and recovery roles—or explicitly accept vacancies.
- [ ] Reconcile Repository `0` and Repository `1` request/quarantine routes.
...
- [ ] Resolve D1–D4 before selecting namespaces, canonical identities, authority bindings, or profile custody.
